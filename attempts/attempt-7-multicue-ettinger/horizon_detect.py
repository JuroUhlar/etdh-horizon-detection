...
3. The winning candidate's inliers are refit with Huber (cv2.fitLine) for
   sub-pixel angle and offset.

4. The detector now abstains ("no_horizon") when:
   - both channels produced a near-degenerate mask (>97% one class), OR
...
_FALLBACK_COHERENCE  = 0.15   # best Ettinger score below this -> abstain
...
def detect_horizon(
    ...
    # Degenerate mask check on both channels: if both are >97% one class, the
    ...
    for entry in pooled:
        ...
        score = _ettinger_score(seed, lab_thumb, w, h)
        if score > best_score:
            best_score = score
            best_entry = entry

    if best_entry is None or best_score < _FALLBACK_COHERENCE:
        return "no_horizon"
    ...
    return _build_result(inlier_pts, confidence, best_score, full_mask)
