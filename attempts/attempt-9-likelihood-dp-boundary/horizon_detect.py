"""
Attempt 9 - multi-cue candidates + temporal prior + likelihood-DP boundary.
...
What's new relative to attempt 8
--------------------------------
...
3. Pooled candidates are re-ranked with an Ettinger-style region-coherence
...
4. When adjacent frames look like the same video scene, candidates close to
...
5. The winning candidate's inliers are refit with Huber (cv2.fitLine) for
   sub-pixel angle and offset.

6. The detector abstains ("no_horizon") when:
...
def detect_horizon(
    ...
    Pipeline:
      ...
      3. Add one low-resolution likelihood-DP boundary candidate if confident.
    ...
    if dp_entry is not None:
        dp_line = dp_entry[2]
        dp_roll = abs(float(np.degrees(np.arctan2(dp_line[1], dp_line[0]))))
        if dp_roll > 90.0:
            dp_roll = 180.0 - dp_roll
